...
export function addNote(contactId: string, body: string, createdBy?: string, db?: ContactsDatabase, companyId?: string): ContactNote {
...
export function listNotes(contactId: string, db?: ContactsDatabase): ContactNote[] {
  const d = db || getDatabase();
  return d.query(
...
export function listNotesForContactAtCompany(contactId: string, companyId: string, db?: ContactsDatabase): ContactNote[] {
  const d = db || getDatabase();
  return d.query(
...
export function deleteNote(noteId: string, db?: ContactsDatabase): void {
  const d = db || getDatabase();
  d.run(`DELETE FROM contact_notes WHERE id = ?`, [noteId]);
}

export function getNote(noteId: string, db?: ContactsDatabase): ContactNote | null {
  const d = db || getDatabase();
